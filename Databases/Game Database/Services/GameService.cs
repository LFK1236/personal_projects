...
namespace Game_Database.Services
{
    public static class GameService
    {
        ...
        static GameService()
        {
            Games = new List<Game>
            {
                new Game { Title = "Star Wars Jedi: Fallen Order", Release_year = "2019" },
                new Game { Title = "Kena: Bridge of Spirits", Release_year = "2021" }
            };
        }

        public static List<Game> GetAll() => Games;

        public static Game? Get(string title, string release) => Games.FirstOrDefault(g => (g.Title == title) && (g.Release_year == release));
